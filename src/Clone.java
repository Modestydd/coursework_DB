...
public class Clone extends DbBasic{
	 ...
	 */
	Clone ( String dbName ) throws SQLException {
		super( dbName );
	...
	public void createAll(String db) throws SQLException, IOException {
		String createClone = "sqlite3 " + db + "Clone.db <create_table_" +db + ".sql \n";
		String insertClone = "sqlite3 " + db + "Clone.db <insert" +db + ".sql \n";
		write(db + ".bat", createClone + insertClone + "\n pause");
		write("CloneAll.bat", createClone + insertClone);
	...
	public void createOrder() throws SQLException {		
		...
		while (rs.next()) {
			...
			if(!tableNames.contains(rs.getString(3)))				// If there is no foreign keys
	...
	public void createSta(String db, String table) throws SQLException, IOException {
		...
		for(int i = 0; i < columnName.size(); i++) {
			...
			create += (nullable.get(i).equals("0"))? (" not null"):("");	// not null
		}
	...
	public void getIndex(String table) throws SQLException {
		...
		while(rs.next()) {			
			if(rs.getString("INDEX_NAME").contains("sqlite_autoindex_"))
				continue;
			if(rs.getString("ORDINAL_POSITION").equals("1")) {			// If the original position is 1
				...
				index += (rs.getString("ASC_OR_DESC") == null)?("\""):((rs.getString("ASC_OR_DESC").equals("A"))? (" ASC\""):(" DESC\""));
				
				createIndex.add(index);
			}
			else {														// If the original position>1
	...
	public void insert(String db, String table) throws SQLException, IOException {
		String s = "SELECT * FROM \""+table+"\";";				// Get data
		...
		while(result.next()) {
			insert1 = insert;
			insert2 = insert;
			for(int i = 1; i <= size; i++) {					// Concat insert statements
	...
	public void write(String file, String content) throws IOException {
		...
		if(!f.exists())										//If file doesn't exists, then create it
			f.createNewFile();
